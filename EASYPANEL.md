...
> **🚀 Рекомендуемый путь**: см. `DEPLOY-OPTIMIZED.md` — деплой через GHCR (готовые образы),
> сервер ничего не собирает, экономит 5–7 ГБ диска.
>
> **Этот файл** описывает старую схему (сборка прямо на сервере EasyPanel).
> Используй её только если нужен авто-билд из GitLab/Gitea без GHCR.
